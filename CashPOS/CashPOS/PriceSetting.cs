    ...
    public partial class PriceSetting : UserControl
    {
        ...
        private void searchByItemBtn_Click(object sender, EventArgs e)
        {
            if (itemList.Text.Length > 0)
            {
                ...
                + " b.Name, b.PayMethod FROM CashPOSDB.custProdPrice a join CashPOSDB.custData b on a.cust	= b.Code and Prod = '" + item + "' and b.Code like '%SF%' order by a.Prod", myConnection);
                myConnection.Open();
                rdr = myCommand.ExecuteReader();
                if (rdr.HasRows)
                {
